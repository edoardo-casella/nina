...
def shelter_score(anch: dict, twd: float, tws: float, gust: float) -> dict:
...
def rank(twd: float, tws: float, gust: float | None = None) -> list[dict]:
    gust = gust if gust is not None else tws * 1.3
    return sorted((shelter_score(a, twd, tws, gust) for a in anchorages()),
                  key=lambda x: x["score"], reverse=True)


def rank_for_tonight(wp_id: str, day: str) -> list[dict]:
    ...
    return rank(night["twd"], night["tws"], night["gust"]), night
...
if __name__ == "__main__":
    ...
    if a.from_wp and a.day:
        rows, night = rank_for_tonight(a.from_wp, a.day)
        print(f"Notte {a.day}: {night['tws']:.0f} kn da {night['twd']:.0f}° (raffiche {night['gust']:.0f})\n")
    else:
        rows = rank(a.twd, a.tws)

    icon = {"verde": "🟢", "giallo": "🟡", "rosso": "🔴"}
    for r in rows:
        print(f"{icon[r['light']]} {r['score']:>3}  {r['name']:<30} {r['why']}")
    print("\n" + weather.AM_NOTE)
